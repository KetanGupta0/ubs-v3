<script setup>
/**
 * Paying one course fee.
 *
 * The amount is never sent from here. The browser asks the server to open
 * checkout and the server prices it from the invoice, because a checkout that
 * reads its amount off the page can settle a full fee with one rupee.
 */
import { ref, onMounted } from 'vue';
import { Head, router } from '@inertiajs/vue3';
import { ArrowLeft, Download, CreditCard, ShieldCheck, AlertTriangle, Lock } from 'lucide-vue-next';
...
onMounted(() => {
    ...
    // Loaded only when a real provider is configured, so a development machine
    // does not call out to a third party on every page view.
...
async function pay() {
    ...
    try {
        const response = await fetch(`/student/payments/${props.payment.id}/checkout`, {
            method: 'POST',
            headers: {
...
function openRazorpay(data) {
    ...
    const checkout = new window.Razorpay({
        ...
        theme: { color: '#4f46e5' },
        handler: (result) => settle(data.transaction, result),
        modal: {
...
function settle(reference, payload) {
    router.post(`/student/payments/${reference}/confirm`, payload, { preserveScroll: true });
}

function simulate(outcome) {
    const data = simulating.value;
    simulating.value = null;
    settle(data.transaction, { outcome });
}
</script>
...
<template>
    ...
    <AppLayout
        ...
        :breadcrumbs="[
            ...
            { label: 'Fees', href: '/student/payments' },
    ...
    >
        <div class="mx-auto max-w-2xl space-y-5">
            <PageHeader :title="payment.title" :description="payment.description">
                <template #actions>
                    <UiButton href="/student/payments" variant="ghost" size="sm">
            ...
            <UiCard>
                ...
                <dl class="mt-5 space-y-1.5 border-t pt-4 text-sm" style="border-color: var(--border-subtle)">
                    <div class="flex justify-between">
                        <dt style="color: var(--text-muted)">Course fee</dt>
                ...
                >
                    ...
                    <span>
                        You are already enrolled, so the syllabus, the schedule and the preview lessons are open.
                        Paying opens the rest of the course straight away.
                    </span>
                </p>
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-xs" style="color: var(--text-muted)">
                            Issued {{ invoice.issuedAt }} · {{ invoice.total }}
                        </p>
                    </div>
        ...
        >
            <p class="flex items-start gap-2 text-sm" style="color: var(--text-muted)">
                ...
                This screen does not appear once payment keys are set.
            </p>
